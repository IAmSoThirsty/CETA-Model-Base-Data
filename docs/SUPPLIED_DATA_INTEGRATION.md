...
The supplied operation-risk ranking is machine-read by `promotion_policy_from_risk_material`. Independent evaluation now emits per-operation case counts, exact-target accuracy, selected-transition operation accuracy, VM-legal selection rate, illegal-selection counts, hostile/surviving candidate counts, target margins, ambiguous top-selection counts, and case-level selection-error diagnostics. A governed epoch uses the supplied operation-specific accuracy floors and zero-illegal-selection requirements in addition to the aggregate promotion policy. Missing operation metrics, singleton evaluation, metric-contract disagreement, and tie-order selection fail closed.
