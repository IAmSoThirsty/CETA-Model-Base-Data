...
The supplied operation-risk ranking is machine-read by `promotion_policy_from_risk_material`. Independent evaluation now emits per-operation case counts, exact-target accuracy, opcode accuracy, VM-legal selection rate, illegal-selection counts, hostile/surviving candidate counts, target margins, and ambiguous top-selection counts. A governed epoch uses the supplied operation-specific accuracy floors and zero-illegal-selection requirements in addition to the aggregate promotion policy. Missing operation metrics, singleton evaluation, and tie-order selection fail closed.
