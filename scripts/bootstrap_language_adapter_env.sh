...
if [[ "${1:-}" == "--target" ]]; then
  ...
  PYTHONPATH="${package_root}" "${python_seed}" -S -c 'import accelerate, bitsandbytes, peft, safetensors, torch, transformers; assert torch.__version__.split("+")[0] == "2.13.0"; assert transformers.__version__ == "5.10.1"; print("CETA LANGUAGE TARGET: PASS", torch.__version__, transformers.__version__)'
  exit 0
fi
